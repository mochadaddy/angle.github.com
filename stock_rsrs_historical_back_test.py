...
for stock_file in read_dir_files:
    df = pd.read_csv(read_dir + os.sep + stock_file, usecols=['ts_code', 'trade_date', 'high', 'low', 'close', 'beta'])
    ...
    for i in range(0, row_num):
        ...
        if i > 0:
            if (df.iloc[i]['beta'] >= 1) and (df.iloc[i-1]['beta'] < 1):
                ...
                while pd.isnull(df.iloc[k]['flag']):
                    ...
                    if df.iloc[k]['flag'] == 0 or k == 0:
                        ...
                        break
            if (df.iloc[i]['beta'] <= 0.85) and (df.iloc[i-1]['beta'] > 0.85):
                j = i
            ...
            continue
    pd.DataFrame.to_csv(df, targetdir + os.sep + stock_file, encoding='gbk')
