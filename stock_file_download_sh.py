...
# 联接tushare的api接口
pro = sc.get_tocken()
targetdir = 'D:/Program Files/tdx/vipdoc/sh/sh_tushare/day_download'
targetdir_week = 'D:/Program Files/tdx/vipdoc/sh/sh_tushare/week_download'
targetdir_cap = 'D:/Program Files/tdx/vipdoc/sh/sh_tushare/sh_capital'
startdate = '20000101'
midday = '20100101'
# 得到系统日期的前一天
formatted_yesterday = sc.get_sys_date()
ma_list = [5, 20, 60]
stock_list = sc.get_sh_stock()
# print stock_list
...
# 获取深证成分指数日线数据
df_index_1 = pro.index_daily(ts_code='000001.SH', start_date=startdate, end_date=midday)
df_index_2 = pro.index_daily(ts_code='000001.SH', start_date=midday, end_date=formatted_yesterday)
frames_index = [df_index_2, df_index_1]
frames_result_index = pd.concat(frames_index)


df_index_week =pro.index_weekly(ts_code='000001.SH', start_date=startdate, end_date=formatted_yesterday, ma=[5])
...
pd.DataFrame.to_csv(df_index, targetdir + os.sep + '000001.SH.csv', encoding='gbk')
pd.DataFrame.to_csv(df_index_week, targetdir_week + os.sep + '000001.SH.csv', encoding='gbk')
