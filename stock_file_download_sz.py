...
for i in range(706, stock_num):
    ...
    for _ in range(3):
        try:
            df_1 = ts.pro_bar(ts_code=stock_code, freq='D', adj='hfq', start_date=startdate, end_date=formatted_yesterday)
            #df_2 = ts.pro_bar(ts_code=stock_code, freq='D', adj='hfq', start_date=midday, end_date=formatted_yesterday)
            if weekday == 5 or weekday == 3:
                df_week_1 = ts.pro_bar(ts_code=stock_code, freq='W', adj='hfq', start_date=startdate, end_date=formatted_yesterday)
    ...
    if df_1.empty:
        ...
        #frames = [df_2, df_1]
        #df_frames_result = pd.concat(frames)
        df = df_1.sort_values(by='trade_date', ascending=True)
    ...
    if df_week_1.empty:
        ...
        df_week_sort = df_week_1.sort_values(by='trade_date', ascending=True)
