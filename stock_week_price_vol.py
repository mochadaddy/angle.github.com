...
for i in range(0, stock_num):
    ...
    df = pd.read_csv(read_dir_week + os.sep + stock_code + '.csv', usecols=['ts_code', 'trade_date', 'open', 'close',
                                                                            'pct_chg', 'vol', 'amount', 'MA_5'])
    ...
    for ma in ma_week_list:
        df_new['amount_' + str(ma)] = df_new['amount'].rolling(window=ma, center=False).mean()
    df_new = df_new.reset_index(drop=True)
    mask1 = df_new['close'] - df_new['MA_5']
    mask2 = df_new['amount'] - df_new['amount_5']
    df_new.loc[(mask1 > 0) & (mask2 > 0), 'flag'] = 1
    #df_new.loc[(mask1 > 0) & (mask2 < 0), 'flag'] = 0
    df_new.loc[mask1 < 0, 'flag'] = 0
    df_num = df.shape[0]
    for k in range(0, df_num):
        if (df_new.iloc[k]['close'] > df_new.iloc[k]['MA_5']) and (df_new.iloc[k]['amount'] < df_new.iloc[k]['amount_5']):
            df_new['flag'].fillna(method='ffill', inplace=True)
...
for stock_file in read_dir_files:
    ...
    for j in range(0, day_trade_num):
        if df1.iloc[j]['close'] == df1.iloc[j]['MA_5']:
            # 对flag列根据前值填充当前列的空值
